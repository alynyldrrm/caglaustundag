@extends('client.layout')

@php
    if(!is_array($list) || count($list) == 0) {
        $list = getTypeValues('hizmetler', 100);
    }
    $list = is_array($list) ? $list : [];
@endphp

{{-- DETAY SAYFASI --}}
@if($detail)
    @section('title', $detail['name'] ?? __('Hizmet Detayı'))

    @section('content')
        @php
            $baslik    = getValue('baslik', $detail);
            $altbaslik = getValue('altbaslik', $detail);
            $icerik    = getValue('icerik', $detail);
        @endphp

        <!-- Hero -->
        <section class="sh-hero">
            <div class="container">
                <div class="sh-hero__inner">
                    <nav class="sh-breadcrumb">
                        <a href="/{{ App::getLocale() }}">{{ __('Ana Sayfa') }}</a>
                        <i class="fas fa-chevron-right"></i>
                        <a href="{{ url()->previous() }}">{{ __('Hizmetler') }}</a>
                        <i class="fas fa-chevron-right"></i>
                        <span>{!! $baslik !!}</span>
                    </nav>
                    <span class="sh-hero__eyebrow">{{ __('Hizmet Detayı') }}</span>
                    <h1 class="sh-hero__title">{!! $baslik !!}</h1>
                    @if($altbaslik)
                        <p class="sh-hero__sub">{!! $altbaslik !!}</p>
                    @endif
                </div>
            </div>
        </section>

        <!-- Content -->
        <section class="sh-detail">
            <div class="container">
                <div class="sh-detail__grid">

                    <!-- Ana İçerik -->
                    <div class="sh-detail__main">

                        @php
                            $contentText = strip_tags($icerik);
                            $paragraphs = array_filter(array_map('trim', explode("\n", $contentText)));
                            $paragraphs = array_values($paragraphs);
                            $intro = $paragraphs[0] ?? '';
                            $otherParas = array_slice($paragraphs, 1);
                        @endphp

                        @if($intro)
                            <p class="sh-intro">{!! $intro !!}</p>
                        @endif

                        @foreach($otherParas as $para)
                            @if(trim($para))
                                @php
                                    $isBold = strpos($para, 'çalıştığınızda:') !== false ||
                                              strpos($para, 'Nedir?') !== false ||
                                              mb_substr($para, -1) === ':';
                                @endphp
                                @if($isBold)
                                    <h3 class="sh-subtitle">{!! $para !!}</h3>
                                @else
                                    <p class="sh-paragraph">{!! $para !!}</p>
                                @endif
                            @endif
                        @endforeach

                        @php
                            preg_match_all('/<li[^>]*>(.*?)<\/li>/s', $icerik, $matches);
                            $items = $matches[1] ?? [];
                            $groups = [];
                            $currentGroup = [];
                            foreach($items as $item) {
                                $text = strip_tags($item);
                                if(strpos($text, 'YATIRIM DÖNEMİ') !== false || strpos($text, 'İŞLETME DÖNEMİ') !== false) {
                                    if(!empty($currentGroup)) $groups[] = $currentGroup;
                                    $currentGroup = ['title' => $text, 'items' => []];
                                } else {
                                    $currentGroup['items'][] = $text;
                                }
                            }
                            if(!empty($currentGroup)) $groups[] = $currentGroup;
                        @endphp

                        @if(!empty($groups))
                            <div class="sh-groups">
                                @foreach($groups as $group)
                                    <div class="sh-group">
                                        @if(!empty($group['title']))
                                            <h3 class="sh-group__title">{!! $group['title'] !!}</h3>
                                        @endif
                                        <div class="sh-group__grid">
                                            @foreach($group['items'] as $item)
                                                <div class="sh-feat">
                                                    <span class="sh-feat__dot"></span>
                                                    <span>{!! $item !!}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        @endif

                        <!-- CTA -->
                        <div class="sh-cta">
                            <div class="sh-cta__left">
                                <span class="sh-cta__icon"><i class="fas fa-phone-alt"></i></span>
                                <div>
                                    <h4>{{ __('Bu hizmet hakkında bilgi alın') }}</h4>
                                    <p>{{ __('Uzman ekibimiz size yardımcı olmaya hazır') }}</p>
                                </div>
                            </div>
                            <a href="{{ route('showPage', [App::getLocale(), 'iletisim']) }}" class="sh-cta__btn">
                                {{ __('Bize Ulaşın') }}
                                <i class="fas fa-arrow-right"></i>
                            </a>
                        </div>
                    </div>

                    <!-- Sidebar -->
                    @if(count($list) > 1)
                        <aside class="sh-sidebar">
                            <div class="sh-sidebar__box">
                                <p class="sh-sidebar__label">{{ __('Diğer Hizmetler') }}</p>
                                <ul class="sh-sidebar__list">
                                    @foreach(array_slice($list, 0, 5) as $hizmet)
                                        @if($hizmet['id'] != ($detail['id'] ?? 0))
                                            @php
                                                $hbaslik         = getValue('baslik', $hizmet);
                                                $hmenuPermalink  = $hizmet['menu']['permalink'] ?? '';
                                                $hlanguageKey    = $hizmet['language']['key'] ?? 'tr';
                                            @endphp
                                            <li>
                                                <a href="{{ route('showPage', [$hlanguageKey, $hmenuPermalink, $hizmet['permalink']]) }}">
                                                    <i class="fas fa-chevron-right"></i>
                                                    <span>{!! $hbaslik !!}</span>
                                                </a>
                                            </li>
                                        @endif
                                    @endforeach
                                </ul>
                            </div>
                        </aside>
                    @endif

                </div>
            </div>
        </section>
    @endsection

{{-- LİSTE SAYFASI --}}
@else
    @section('title', getSelectedMenus()->last() ? getSelectedMenus()->last()->name : __('Hizmetler'))

    @section('content')

        <!-- Hero -->
        <section class="sl-hero">
            <div class="container">
                <div class="sl-hero__inner">
                    <span class="sl-hero__eyebrow">{{ __('Hizmetlerimiz') }}</span>
                    <h1 class="sl-hero__title">{{ __('Profesyonel') }} <em>{{ __('Çözümler') }}</em></h1>
                    <p class="sl-hero__sub">{{ __('İşletmenizin büyümesi için sunduğumuz danışmanlık hizmetleri') }}</p>
                </div>
            </div>
        </section>

        <!-- List -->
        <section class="sl-main">
            <div class="container">
                @if(count($list) > 0)
                    <div class="sl-grid">
                        @foreach($list as $index => $hizmet)
                            @php
                                $baslik         = getValue('baslik', $hizmet);
                                $altbaslik      = getValue('altbaslik', $hizmet);
                                $menuPermalink  = $hizmet['menu']['permalink'] ?? '';
                                $languageKey    = $hizmet['language']['key'] ?? 'tr';
                            @endphp
                            <article class="sl-card">
                                <div class="sl-card__top">
                                    <span class="sl-card__num">{{ sprintf('%02d', $index + 1) }}</span>
                                    <div class="sl-card__dash"></div>
                                </div>
                                <h3 class="sl-card__title">{!! $baslik !!}</h3>
                                @if($altbaslik)
                                    <p class="sl-card__desc">{!! $altbaslik !!}</p>
                                @endif
                                <a href="{{ route('showPage', [$languageKey, $menuPermalink, $hizmet['permalink']]) }}" class="sl-card__link">
                                    {{ __('Detayı İncele') }}
                                    <span class="sl-card__arr"><i class="fas fa-arrow-right"></i></span>
                                </a>
                            </article>
                        @endforeach
                    </div>
                @else
                    <div class="sl-empty">
                        <p>{{ __('Henüz hizmet eklenmemiş.') }}</p>
                    </div>
                @endif
            </div>
        </section>

    @endsection
@endif

@section('css')
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;1,400&family=Nunito:wght@400;500;600;700&display=swap" rel="stylesheet">

<style>
*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

:root {
    --bg:       #f5f4f0;
    --white:    #ffffff;
    --ink:      #181818;
    --ink-2:    #4a4a45;
    --ink-3:    #9a9891;
    --border:   #e2e0d8;
    --border-2: #c8c5ba;
    --accent:   #1e3a4f;
    --accent-l: rgba(30,58,79,0.06);
    --accent-m: rgba(30,58,79,0.13);
    --gold:     #b5904a;
}

.container {
    width: min(1120px, 92vw);
    margin-inline: auto;
    font-family: 'Nunito', sans-serif;
    -webkit-font-smoothing: antialiased;
}

/* ══════════════════════════════════
   HERO — kompakt, boşluksuz
══════════════════════════════════ */
.sh-hero, .sl-hero {
    background: var(--white);
    padding: 48px 0 40px;
    border-bottom: 1px solid var(--border);
    font-family: 'Nunito', sans-serif;
}

.sh-hero__inner, .sl-hero__inner {
    display: flex;
    flex-direction: column;
    gap: 6px;
}

/* Eyebrow */
.sh-hero__eyebrow, .sl-hero__eyebrow {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    font-size: .75rem;
    font-weight: 700;
    letter-spacing: .18em;
    text-transform: uppercase;
    color: var(--gold);
}

.sh-hero__eyebrow::before, .sl-hero__eyebrow::before {
    content: '';
    display: block;
    width: 20px; height: 1px;
    background: var(--gold);
}

/* Başlıklar */
.sh-hero__title {
    font-family: 'Playfair Display', serif;
    font-size: clamp(2rem, 4vw, 3.2rem);
    font-weight: 500;
    line-height: 1.12;
    color: var(--ink);
    letter-spacing: -.015em;
}

.sl-hero__title {
    font-family: 'Playfair Display', serif;
    font-size: clamp(2.2rem, 4.5vw, 3.6rem);
    font-weight: 500;
    line-height: 1.12;
    color: var(--ink);
    letter-spacing: -.015em;
}

.sl-hero__title em {
    font-style: italic;
    color: var(--accent);
}

.sh-hero__sub, .sl-hero__sub {
    margin-top: 6px;
    font-size: 1rem;
    color: var(--ink-2);
    font-weight: 400;
}

/* ══════════════════════════════════
   BREADCRUMB
══════════════════════════════════ */
.sh-breadcrumb {
    display: flex;
    align-items: center;
    gap: 7px;
    margin-bottom: 14px;
    font-size: .82rem;
    font-family: 'Nunito', sans-serif;
}

.sh-breadcrumb a { color: var(--ink-3); text-decoration: none; transition: color .2s; }
.sh-breadcrumb a:hover { color: var(--accent); }
.sh-breadcrumb i { font-size: .48rem; color: var(--border-2); }
.sh-breadcrumb span { color: var(--ink-2); font-weight: 600; }

/* ══════════════════════════════════
   DETAY LAYOUT
══════════════════════════════════ */
.sh-detail {
    padding: 52px 0 80px;
    background: var(--bg);
    font-family: 'Nunito', sans-serif;
}

.sh-detail__grid {
    display: grid;
    grid-template-columns: 1fr 260px;
    gap: 44px;
    align-items: start;
}

/* Intro */
.sh-intro {
    font-size: 1.08rem;
    color: var(--ink-2);
    line-height: 1.85;
    font-weight: 400;
    margin-bottom: 22px;
    padding-bottom: 28px;
    border-bottom: 1px solid var(--border);
}

/* Paragraflar */
.sh-paragraph {
    font-size: 1rem;
    color: var(--ink-2);
    line-height: 1.8;
    font-weight: 400;
    margin-bottom: 16px;
}

/* Alt başlıklar */
.sh-subtitle {
    font-size: 1.05rem;
    font-weight: 700;
    color: var(--ink);
    margin: 24px 0 12px;
    font-family: 'Nunito', sans-serif;
}

/* Feature groups */
.sh-groups { display: flex; flex-direction: column; gap: 22px; }

.sh-group {
    background: var(--white);
    border: 1px solid var(--border);
    border-radius: 10px;
    padding: 24px 26px;
}

.sh-group__title {
    font-size: .72rem;
    font-weight: 700;
    letter-spacing: .14em;
    text-transform: uppercase;
    color: var(--accent);
    margin-bottom: 16px;
    padding-bottom: 12px;
    border-bottom: 1px solid var(--border);
}

.sh-group__grid {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 10px;
}

.sh-feat {
    display: flex;
    align-items: flex-start;
    gap: 10px;
    padding: 10px 12px;
    background: var(--bg);
    border-radius: 7px;
    font-size: .93rem;
    color: var(--ink-2);
    line-height: 1.5;
    font-family: 'Nunito', sans-serif;
    font-weight: 400;
    transition: background .2s;
}

.sh-feat:hover { background: var(--accent-l); }

.sh-feat__dot {
    width: 6px; height: 6px;
    border-radius: 50%;
    background: var(--gold);
    flex-shrink: 0;
    margin-top: 6px;
}

/* CTA */
.sh-cta {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 20px;
    margin-top: 36px;
    padding: 24px 28px;
    background: var(--accent);
    border-radius: 10px;
    color: #fff;
}

.sh-cta__left { display: flex; align-items: center; gap: 16px; }

.sh-cta__icon {
    width: 44px; height: 44px;
    border-radius: 9px;
    border: 1px solid rgba(255,255,255,.2);
    display: flex; align-items: center; justify-content: center;
    font-size: .95rem;
    flex-shrink: 0;
}

.sh-cta h4 {
    font-size: 1rem;
    font-weight: 600;
    margin-bottom: 3px;
    font-family: 'Nunito', sans-serif;
}

.sh-cta p {
    font-size: .88rem;
    color: rgba(255,255,255,.6);
    font-weight: 400;
    font-family: 'Nunito', sans-serif;
}

.sh-cta__btn {
    display: inline-flex;
    align-items: center;
    gap: 9px;
    padding: 11px 22px;
    background: var(--white);
    color: var(--accent);
    border-radius: 7px;
    font-family: 'Nunito', sans-serif;
    font-size: .93rem;
    font-weight: 700;
    text-decoration: none;
    white-space: nowrap;
    transition: all .2s;
}

.sh-cta__btn:hover {
    background: var(--bg);
    box-shadow: 0 4px 16px rgba(0,0,0,.1);
    text-decoration: none;
    color: var(--accent);
}

.sh-cta__btn i { transition: transform .2s; }
.sh-cta__btn:hover i { transform: translateX(4px); }

/* Sidebar */
.sh-sidebar { position: sticky; top: 90px; }

.sh-sidebar__box {
    background: var(--white);
    border: 1px solid var(--border);
    border-radius: 10px;
    padding: 22px;
}

.sh-sidebar__label {
    font-size: .7rem;
    font-weight: 700;
    letter-spacing: .14em;
    text-transform: uppercase;
    color: var(--gold);
    margin-bottom: 14px;
    font-family: 'Nunito', sans-serif;
}

.sh-sidebar__list { list-style: none; }

.sh-sidebar__list li { border-bottom: 1px solid var(--border); }
.sh-sidebar__list li:last-child { border-bottom: none; }

.sh-sidebar__list a {
    display: flex;
    align-items: center;
    gap: 9px;
    padding: 11px 0;
    color: var(--ink-2);
    text-decoration: none;
    font-size: .93rem;
    font-family: 'Nunito', sans-serif;
    font-weight: 500;
    transition: all .18s;
}

.sh-sidebar__list a:hover { color: var(--accent); padding-left: 4px; }
.sh-sidebar__list i { font-size: .5rem; color: var(--border-2); transition: color .18s; }
.sh-sidebar__list a:hover i { color: var(--accent); }

/* ══════════════════════════════════
   LİSTE — GRID
══════════════════════════════════ */
.sl-main {
    padding: 52px 0 80px;
    background: var(--bg);
    font-family: 'Nunito', sans-serif;
}

.sl-grid {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(290px, 1fr));
    gap: 20px;
}

.sl-card {
    background: var(--white);
    border: 1px solid var(--border);
    border-radius: 10px;
    padding: 26px 24px;
    display: flex;
    flex-direction: column;
    transition: transform .24s ease, box-shadow .24s ease, border-color .24s ease;
    position: relative;
    overflow: hidden;
}

.sl-card::before {
    content: '';
    position: absolute;
    top: 0; left: 0; right: 0;
    height: 2px;
    background: linear-gradient(90deg, var(--gold), var(--accent));
    transform: scaleX(0);
    transform-origin: left;
    transition: transform .28s ease;
}

.sl-card:hover {
    transform: translateY(-4px);
    box-shadow: 0 14px 40px rgba(30,58,79,.1);
    border-color: var(--border-2);
}

.sl-card:hover::before { transform: scaleX(1); }

.sl-card__top {
    display: flex;
    align-items: center;
    gap: 12px;
    margin-bottom: 16px;
}

.sl-card__num {
    font-family: 'Playfair Display', serif;
    font-size: 1.3rem;
    font-weight: 400;
    color: var(--gold);
    opacity: .6;
    line-height: 1;
}

.sl-card__dash { flex: 1; height: 1px; background: var(--border); }

.sl-card__title {
    font-family: 'Playfair Display', serif;
    font-size: 1.35rem;
    font-weight: 500;
    color: var(--ink);
    line-height: 1.25;
    margin-bottom: 10px;
    letter-spacing: -.01em;
}

.sl-card__desc {
    font-size: .95rem;
    color: var(--ink-2);
    line-height: 1.7;
    font-weight: 400;
    flex: 1;
    margin-bottom: 22px;
}

.sl-card__link {
    display: inline-flex;
    align-items: center;
    gap: 9px;
    font-size: .78rem;
    font-weight: 700;
    letter-spacing: .08em;
    text-transform: uppercase;
    color: var(--accent);
    text-decoration: none;
    margin-top: auto;
    transition: gap .2s;
}

.sl-card__link:hover { gap: 13px; text-decoration: none; }

.sl-card__arr {
    width: 26px; height: 26px;
    border: 1px solid var(--accent-m);
    border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: .62rem;
    transition: background .2s, border-color .2s, color .2s;
}

.sl-card__link:hover .sl-card__arr {
    background: var(--accent);
    border-color: var(--accent);
    color: #fff;
}

/* Empty */
.sl-empty {
    text-align: center;
    padding: 80px 20px;
    color: var(--ink-3);
    font-size: 1rem;
    font-family: 'Nunito', sans-serif;
}

/* ══════════════════════════════════
   RESPONSIVE
══════════════════════════════════ */
@media (max-width: 1024px) {
    .sh-detail__grid { grid-template-columns: 1fr; }
    .sh-sidebar { position: static; }
    .sh-group__grid { grid-template-columns: 1fr; }
    .sh-cta { flex-direction: column; text-align: center; }
    .sh-cta__left { flex-direction: column; text-align: center; }
}

@media (max-width: 768px) {
    .sh-hero, .sl-hero { padding: 36px 0 30px; }
    .sl-main, .sh-detail { padding: 40px 0 60px; }
    .sl-grid { grid-template-columns: 1fr; }
    .sh-group { padding: 18px; }
}
</style>
@endsection
